...
get.cluster.correlations <- function(de_results, template_results, cor.method = "spearman"){
	results_list <- list()
	...
	for(i in 1:length(de_clusters)){
		...
		for(j in 1:length(template_clusters)){
			...
			if(length(gene_list) > 2){
				cor <- cor.test(
					temp_template_de_results[gene_list, "score"], 
					temp_de_results[gene_list, "score"], 
					method = cor.method)
				hyper_p <- phyper(
					length(gene_list)-1, 
...
combine.results <- function(results_list){
...
final_res <- combine.results(list(allen = cor_res))
